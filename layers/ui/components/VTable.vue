  ...
  <div class="overflow-x-auto hide-scrollbar" :class="wrapperClass">
    <table class="w-full text-start" :class="tableClass">
      <thead v-if="headers && headers.length"
        class="text-[9px] font-black text-tx-muted tracking-widest border-b border-border-subtle bg-bg-overlay">
        <tr>
          <th v-for="header in headers" :key="header.label" :class="[
            'px-6 py-4',
      ...
      <tbody class="divide-y divide-border-subtle">
        <tr v-for="(item, index) in items" :key="item.id || index" class="group hover:bg-bg-elevated transition-colors"
          @click="$emit('row-click', item)">
          <td v-for="header in headers" :key="header.label" :class="[
            'px-6 py-4',
            header.align === 'center' ? 'text-center' : header.align === 'end' ? 'text-end' : 'text-start',
            header.class || ''
          ]">
        ...
        <tr v-if="!items || items.length === 0">
          <td :colspan="headers.length" class="px-6 py-12 text-center">
            <div class="flex flex-col items-center gap-2 text-tx-muted">
