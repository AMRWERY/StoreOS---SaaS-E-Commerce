<template>
  <div>
    <div class="space-y-2.5">
      ...
      <div
        v-else
        class="relative border border-border-default rounded-xl bg-bg-overlay overflow-hidden group"
      >
        <!-- Image Preview -->
        ...
        <div v-else class="p-6 flex items-center gap-4">
          <div
            class="w-12 h-12 bg-brand/10 rounded-xl flex items-center justify-center shrink-0"
          >
            <Icon name="ph:file-bold" class="text-brand text-2xl" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold truncate">{{ selectedFile.name }}</p>
            <p class="text-[11px] text-tx-secondary">
...
import type { VFileUploadProps } from "../types/VFileUpload";
...
const props = withDefaults(defineProps<VFileUploadProps>(), {
  ...
  wrapperClass:
    "relative border-2 border-dashed border-border-subtle rounded-xl p-5 flex flex-col items-center justify-center hover:bg-bg-elevated hover:border-border-default transition-all cursor-pointer group",
  iconWrapperClass:
    "w-12 h-12 bg-brand/10 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform",
