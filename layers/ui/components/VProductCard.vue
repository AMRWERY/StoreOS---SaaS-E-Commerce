<template>
  <div :class="[
    'bg-bg-primary border border-border-subtle rounded-xl overflow-hidden group hover:border-border-default transition-all cursor-pointer flex',
    ...
    <div class="p-6 flex flex-col flex-1">
      <div class="flex justify-between items-start mb-2">
        <h3 class="font-bold text-sm text-tx-secondary group-hover:text-tx-primary transition-colors truncate pe-2">
          {{ product.name }}
        </h3>
      ...
      <div class="flex items-center justify-between mt-auto">
        <span class="text-[9px] font-black text-tx-muted tracking-[0.2em]">{{ product.category }}</span>
