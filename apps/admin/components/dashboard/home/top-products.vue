      ...
      <div class="space-y-6">
        <div v-for="(prod, i) in topProducts" :key="prod.name" class="flex items-center gap-4 group cursor-pointer">
          <div
            ...
            <img :src="publicImageUrl(prod.image)" :alt="prod.name" width="48" height="48" loading="lazy"
              decoding="async"
              class="size-12 object-cover grayscale opacity-60 transition-all group-hover:grayscale-0 group-hover:opacity-100" />
          </div>
          <div class="flex-1 min-w-0">
            <h4 class="text-sm font-bold truncate group-hover:text-brand transition-colors">
              {{ prod.name }}
            </h4>
            <p class="text-[10px] text-tx-muted font-bold mt-0.5 tracking-tight">
