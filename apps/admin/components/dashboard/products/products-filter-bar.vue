  ...
  <div>
    <div
      ...
      <div class="flex flex-wrap items-center gap-3">
        <LazyVDropdownMenu width="w-48" align="start">
          <template #trigger="{ open }">
            <LazyVButton variant="none" :className="`flex items-center gap-2 px-4 py-2 bg-bg-overlay border border-border-subtle rounded-xl text-xs font-bold transition-all ${open
      ...
      <div class="flex items-center bg-bg-overlay p-1 rounded-xl border border-border-subtle">
        <LazyVButton variant="none" @click="$emit('update:viewMode', 'grid')" :className="`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-brand/20 text-brand' : 'text-tx-muted'
          }`">
          <Icon name="ph:grid-four-fill" />
        </LazyVButton>
        <LazyVButton variant="none" @click="$emit('update:viewMode', 'list')" :className="`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-brand/20 text-brand' : 'text-tx-muted'
          }`">
