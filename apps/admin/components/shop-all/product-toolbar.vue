<template>
  <div
    class="flex items-center justify-between bg-bg-primary border border-border-subtle rounded-[12px] p-2 px-4 shadow-xl"
  >
    <div class="flex gap-1 p-1 bg-black/20 rounded-[8px]">
      <button
        ...
        :class="
          viewMode === 'grid'
            ? 'bg-border-subtle text-brand shadow-lg shadow-[#6366F1]/10'
            : 'text-tx-muted'
        "
      ...
      >
        ...
        <span class="text-[11px] font-bold tracking-widest">{{
          t("storefront.shop.list")
        }}</span>
      </button>
    </div>

    <div class="flex items-center gap-4">
      <span class="text-[10px] font-black text-tx-muted tracking-widest">{{
        t("storefront.shop.sortBy")
      }}</span>
      <div class="relative group">
        ...
        <!-- Mock Dropdown -->
        <div
          class="absolute top-full end-0 mt-2 w-48 bg-bg-primary border border-border-subtle rounded-xl shadow-2xl opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all z-50 p-2"
        >
          <div
            v-for="opt in sortOptions"
            :key="opt"
            @click="$emit('update:selectedSort', opt)"
