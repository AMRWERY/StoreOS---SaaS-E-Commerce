<template>
  <div class="min-h-screen bg-white font-sans antialiased">
    <!-- Preview bar -->
    <div
      class="sticky top-0 z-50 flex items-center justify-between gap-4 border-b border-neutral-200 bg-white px-6 py-2.5 shadow-sm print:hidden"
    >
      <div class="flex items-center gap-3">
        <div
          class="flex size-6 items-center justify-center rounded bg-indigo-600 text-white"
        >
        ...
        <span
          class="rounded-full bg-emerald-100 px-2 py-0.5 text-[9px] font-black tracking-widest text-emerald-700"
        >
          LIVE PREVIEW
        </span>
      </div>
    ...
    <div
      v-if="!hasState"
      class="flex min-h-[60vh] flex-col items-center justify-center gap-4 text-center p-8"
    >
      <div
        class="flex size-20 items-center justify-center rounded-2xl border border-neutral-200 bg-neutral-50"
      >
    ...
    <template v-else>
      <!-- Navbar -->
      <div
        v-if="store.navbarStyle === 'classic'"
        class="flex items-center justify-between border-b border-neutral-100 bg-white px-8 py-4"
      >
        ...
        <nav class="hidden gap-8 text-sm text-neutral-600 md:flex">
          <span
            ...
            :key="link.label"
            class="cursor-pointer transition-colors hover:text-neutral-900"
            >{{ link.label }}</span
          >
        </nav>
      ...
      <div
        v-else-if="store.navbarStyle === 'centered'"
      ...
      >
        ...
        <nav
          class="flex items-center justify-center gap-8 border-t border-neutral-100 py-2.5 text-sm text-neutral-600"
        >
          <span
            v-for="link in store.navLinks"
            :key="link.label"
      ...
      <div
        v-else-if="store.navbarStyle === 'dark'"
        class="flex items-center justify-between bg-neutral-900 px-8 py-4"
      >
      ...
      <div v-else class="border-b border-neutral-100 bg-white px-8 py-5">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-5">
            <nav
              class="hidden gap-7 text-sm font-medium text-neutral-500 md:flex"
            >
              <span
                v-for="link in store.navLinks"
                :key="link.label"
            ...
            <div class="flex items-center gap-2 text-neutral-400">
              <Icon name="ph:magnifying-glass-bold" /><Icon
                name="ph:bag-bold"
              />
            </div>
          </div>
      ...
      <!-- Footer -->
      <div
        v-if="store.footerStyle === 'columns'"
        class="bg-neutral-900 px-8 py-10"
      >
        <div class="flex flex-col gap-8 md:flex-row md:justify-between">
          <div class="md:w-1/3">
            ...
            <div
              ...
              class="mt-4 flex gap-3 text-neutral-400"
            >
              <Icon name="ph:instagram-logo-bold" class="text-lg" /><Icon
                name="ph:twitter-logo-bold"
                class="text-lg"
          ...
          <div class="flex flex-wrap gap-10">
            <div v-for="col in store.footerColumns" :key="col.title">
              <p class="text-xs font-bold tracking-wider text-neutral-400">
                {{ col.title }}
              </p>
              <div class="mt-3 space-y-2">
                <p
                  ...
                  :key="link.label"
                  class="cursor-pointer text-sm text-neutral-500 hover:text-white"
                >
                  {{ link.label }}
                </p>
              </div>
        ...
        <div
          class="mt-8 border-t border-neutral-800 pt-5 text-xs text-neutral-600"
        >
          {{ store.footerCopyright }}
        </div>
      </div>

      <div
        v-else-if="store.footerStyle === 'centered'"
        class="bg-neutral-50 px-8 py-12 text-center"
      >
        <p class="text-xl font-black tracking-tight text-neutral-900">
          {{ store.storeName }}
        </p>
        <p class="mt-2 text-sm text-neutral-500">{{ store.footerTagline }}</p>
        <nav
          class="mt-6 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-sm text-neutral-600"
        >
        ...
        <div
          v-if="store.footerShowSocial"
          class="mt-6 flex items-center justify-center gap-4 text-neutral-400"
        >
          <Icon name="ph:instagram-logo-bold" class="text-xl" /><Icon
            name="ph:twitter-logo-bold"
            class="text-xl"
          /><Icon name="ph:facebook-logo-bold" class="text-xl" />
        </div>
        <div
          class="mt-8 border-t border-neutral-200 pt-5 text-xs text-neutral-400"
        >
          {{ store.footerCopyright }}
        </div>
      </div>
      ...
      <div
        v-else-if="store.footerStyle === 'newsletter'"
        class="bg-neutral-900 px-8 py-10"
      >
        <div
        ...
        >
          ...
          <div
            class="shrink-0 rounded-2xl border border-neutral-700/60 bg-neutral-800 p-6 md:w-80"
          >
      ...
      <div v-else class="border-t border-neutral-200 bg-white px-8 py-5">
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          <nav
            class="flex flex-wrap gap-x-6 gap-y-1.5 text-sm text-neutral-500"
          >
            <template v-for="col in store.footerColumns" :key="col.title">
              <span
                v-for="link in col.links"
