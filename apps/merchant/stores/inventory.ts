...
export const useInventoryStore = defineStore("inventory", () => {
  ...
  const historyList = ref<HistoryLog[]>([
    {
      id: 101,
      date: "Mar 22, 14:30",
      product: "Pro-X Wireless Headset",
      type: "Remove",
      change: -2,
      newStock: 0,
      reason: "Sale",
  ...
  const filteredStockList = computed(() => {
    return stockList.value.filter((item) => {
      ...
      if (selectedStatus.value === "Out of Stock")
        matchesStatus = item.current === 0;
      else if (selectedStatus.value === "Low Stock")
        matchesStatus = item.current > 0 && item.current <= item.threshold;
      else if (selectedStatus.value === "In Stock")
        matchesStatus = item.current > item.threshold;
  ...
  const lowStockCount = computed(
    () =>
      stockList.value.filter((s) => s.current > 0 && s.current <= s.threshold)
        .length,
  );
  const outOfStockCount = computed(
    () => stockList.value.filter((s) => s.current === 0).length,
  );
