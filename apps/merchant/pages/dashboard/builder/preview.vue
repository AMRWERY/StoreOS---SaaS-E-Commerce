<template>
  <div class="min-h-screen bg-white font-sans antialiased">
    ...
    <template v-else>
      <!-- Navbar -->
      <div v-if="store.navbarStyle === 'classic'"
        ...
        <nav class="hidden gap-8 text-sm text-neutral-600 md:flex">
          <span v-for="link in store.navLinks" :key="link.label"
            class="cursor-pointer transition-colors hover:text-neutral-900">{{ link.label }}</span>
        </nav>
      ...
      <div v-else-if="store.navbarStyle === 'dark'" class="flex items-center justify-between bg-neutral-900 px-8 py-4">
        ...
        <nav class="hidden gap-8 text-sm text-neutral-300 md:flex">
          <span v-for="link in store.navLinks" :key="link.label"
            class="cursor-pointer transition-colors hover:text-white">{{ link.label }}</span>
        </nav>
      ...
      <!-- Sections -->
      <main>
        <section-render v-for="section in visibleSections" :key="section.id" :section="section" />
      </main>
      ...
      <div v-if="store.footerStyle === 'columns'" class="bg-neutral-900 px-8 py-10">
          ...
          <div class="flex flex-wrap gap-10">
            <div v-for="col in store.footerColumns" :key="col.title">
              ...
              <div class="mt-3 space-y-2">
                <p v-for="link in col.links" :key="link.label"
        ...
        <div class="mt-8 border-t border-neutral-800 pt-5 text-xs text-neutral-600">
      ...
      <div v-else-if="store.footerStyle === 'centered'" class="bg-neutral-50 px-8 py-12 text-center">
        ...
        <nav class="mt-6 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-sm text-neutral-600">
          <template v-for="col in store.footerColumns" :key="col.title">
            <span v-for="link in col.links" :key="`${col.title}-${link.label}`"
              class="cursor-pointer transition-colors hover:text-neutral-900">{{ link.label }}</span>
          </template>
        </nav>
        ...
        <div class="mt-8 border-t border-neutral-200 pt-5 text-xs text-neutral-400">
          {{ store.footerCopyright }}
        </div>
      </div>
        ...
        <div class="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <div class="mt-5 flex flex-wrap gap-x-6 gap-y-2">
              <template v-for="col in store.footerColumns" :key="col.title">
                <span v-for="link in col.links" :key="link.label"
                  class="cursor-pointer text-sm text-neutral-500 hover:text-white">{{ link.label }}</span>
              </template>
            </div>
          </div>
