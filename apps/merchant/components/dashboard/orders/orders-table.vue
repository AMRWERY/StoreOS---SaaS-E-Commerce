    ...
    <div
      ...
      <div class="col-span-3 flex items-center gap-4">
        <div @click="$emit('select-all')"
          class="w-4 h-4 border rounded cursor-pointer transition-all flex items-center justify-center" :class="isAllSelected
      ...
      <div class="col-span-2 text-center">
        {{ t("dashboard.orders.itemsTotal") }}
      </div>
      <div class="col-span-2 text-center">
        {{ t("dashboard.orders.paymentCol") }}
      </div>
      <div class="col-span-2 text-center">
        {{ t("dashboard.orders.statusCol") }}
      </div>
      ...
      <div v-for="order in orders" :key="order.id" @click="$emit('select-order', order)"
        ...
        :class="[
          order.orderStatus === 'NEW'
            ? 'border-brand/30 ring-1 ring-accent/10 border-s-[3px] border-s-orange-500 shadow-[0_0_15px_-5px_rgba(249,115,22,0.1)]'
            : 'border-border-subtle hover:border-border-default',
          order.active ? 'bg-bg-elevated' : '',
        ]">
        <div class="col-span-3 flex items-center gap-4">
          <div @click.stop="$emit('toggle-selection', order.id)"
            class="w-4 h-4 border rounded transition-all flex items-center justify-center shrink-0" :class="selectedIds.includes(order.id)
              ? 'bg-brand border-brand'
              : 'border-border-default bg-bg-elevated group-hover:border-border-default'
              ">
            <Icon v-if="selectedIds.includes(order.id)" name="ph:check-bold" class="text-[10px] text-tx-primary" />
          </div>
          <LazyVButton variant="none" :to="`/dashboard/orders/${order.id}`" @click.stop
          ...
          <div class="flex flex-col">
            ...
            <span class="text-[10px] text-tx-secondary font-medium">{{
              order.phone
            }}</span>
          </div>
        </div>
        ...
        <div class="col-span-2 flex flex-col items-center">
          ...
          <span class="text-[10px] text-tx-secondary font-black">{{ order.itemsCount }} {{ t("dashboard.orders.items")
          }}</span>
        </div>
        ...
        <div class="col-span-2 flex items-center justify-end gap-2">
          ...
          <LazyVButton variant="none" className="p-2 text-tx-secondary hover:text-tx-primary transition-colors"
            @click.stop="">
