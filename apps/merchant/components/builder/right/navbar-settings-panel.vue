    ...
    <div class="max-h-[min(60vh,520px)] space-y-6 overflow-y-auto px-5 py-5 xl:max-h-[calc(100vh-14rem)]">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <button v-for="style in NAVBAR_STYLES" :key="style.value" type="button"
            ...
            <!-- Mini preview sketch -->
            <div class="mb-2.5 w-full overflow-hidden rounded-lg border border-border-subtle bg-white"
              :class="style.dark ? 'bg-neutral-900' : 'bg-white'">
              <component :is="style.sketch" />
            </div>
            <p class="text-[11px] font-bold"
              :class="store.navbarStyle === style.value ? 'text-brand' : 'text-tx-primary'">
              {{ style.label }}
            </p>
            <p class="mt-0.5 text-[10px] text-tx-muted">{{ style.description }}</p>
            <div v-if="store.navbarStyle === style.value"
      ...
      <div>
        ...
        <input type="text" :value="store.storeName"
          ...
          placeholder="My Store" @input="store.storeName = ($event.target as HTMLInputElement).value" />
      ...
      <div>
        ...
        <p v-if="store.navLinks.length === 0"
