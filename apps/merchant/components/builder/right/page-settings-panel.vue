...
const title = computed({
  ...
  set: (v: string) => {
    if (page.value) page.value.title = v;
  },
});
...
const slug = computed({
  get: () => page.value?.slug ?? "",
  set: (v: string) => {
    if (page.value) page.value.slug = v;
  },
});
...
const showNav = computed({
  ...
  set: (v: boolean) => {
    if (page.value) page.value.show_in_nav = v;
  },
});
...
const seoTitle = computed({
  ...
  set: (v: string) => {
    if (page.value) page.value.seo_title = v || null;
  },
});
...
const seoDesc = computed({
  ...
  set: (v: string) => {
    if (page.value) page.value.seo_desc = v || null;
  },
});
</script>