<template>
  <div class="space-y-2">
    <LazyVInput
      ...
      input-class="!w-full !rounded-md !border-border-subtle !bg-bg-elevated !py-2 !ps-9 !pe-3 !text-[12px] !text-tx-primary !outline-none focus:!border-brand"
    >
      <template #prefix>
        <Icon
    ...
    <ul
      class="max-h-36 space-y-1 overflow-y-auto rounded-md border border-border-subtle bg-bg-base p-2"
    >
      <li
        v-for="p in filtered"
        :key="p.id"
        ...
        :class="
          modelValue === p.id ? 'bg-brand/20 text-brand' : 'text-tx-secondary'
        "
