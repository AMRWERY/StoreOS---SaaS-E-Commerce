        ...
        <div class="flex h-full min-h-[520px] flex-col overflow-hidden rounded-t-xl bg-white">
          ...
          <div class="flex shrink-0 items-center gap-3 border-b border-neutral-200/90 bg-neutral-100/95 px-3 py-2.5">
            ...
            <div
              ...
              <span class="truncate">mystore.storeos.io/{{
          ...
          <div class="relative min-h-0 flex-1 overflow-y-auto bg-white text-neutral-900">
            ...
            <template v-else>
              ...
              <!-- Sections -->
              <section-preview-block v-for="section in store.sections" :key="section.id" :section="section"
                :selected="store.selectedId === section.id" @select="store.selectSection(section.id)"
                @toolbar-action="onToolbar($event, section.id)" />

              <div v-if="store.sections.length === 0"
