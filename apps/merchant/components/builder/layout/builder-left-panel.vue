<template>
  <Transition name="sb-fade">
    <div
      v-if="isOpen"
      class="fixed inset-0 z-40 bg-bg-overlay backdrop-blur-sm lg:hidden"
      aria-hidden="true"
      @click="$emit('close')"
    />
  </Transition>
  ...
  >
    <div
      class="flex items-center gap-1 border-b border-border-subtle bg-bg-base p-1"
    >
      <div class="flex min-w-0 flex-1 gap-0.5">
        <component
          ...
          variant="none"
          type="button"
          :class-name="sidebarTabClass(tab.id)"
      ...
      <component
        ...
        variant="none"
        type="button"
        class-name="shrink-0 p-1.5 text-tx-secondary hover:text-tx-primary lg:hidden"
        title="Close menu"
        @click="$emit('close')"
      >
