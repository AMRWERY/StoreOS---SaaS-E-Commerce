<template>
  <div class="grid min-h-[320px] grid-cols-2">
    <div
      :class="str('image_side', 'right') === 'left' ? 'order-last' : ''"
      class="flex flex-col justify-center bg-neutral-900 px-10 py-12"
    >
      ...
      <LazyVButton
        v-if="str('cta_text', 'Learn More')"
        variant="none"
        className="mt-6 w-fit rounded-md bg-white px-6 py-2.5 text-xs font-bold text-neutral-900"
      >
        {{ str("cta_text", "Learn More") }}
      </LazyVButton>
    </div>
    <div
      class="bg-cover bg-center bg-neutral-700"
      :style="{
        backgroundImage: `url('${str('image_url') || 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800&q=80'}')`,
      }"
    />
  </div>
</template>
