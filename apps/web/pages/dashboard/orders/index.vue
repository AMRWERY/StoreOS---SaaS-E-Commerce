<template>
  <div>
    <div class="flex flex-col h-full overflow-hidden -mt-5">
      ...
      <div
        v-else
        class="flex-1 flex flex-col items-center justify-center px-4 py-3.5 text-center"
      >
        <div
          class="w-64 h-64 bg-bg-elevated rounded-full flex items-center justify-center mb-6 relative"
        >
          <Icon name="ph:shopping-cart-light" class="text-8xl text-tx-muted" />
          <div
            class="absolute -top-2 -end-2 w-16 h-16 bg-brand-dim rounded-full blur-xl"
          ></div>
        </div>
        <h2 class="text-xl font-bold text-tx-primary mb-2">
        ...
        <div
          class="flex items-center gap-3 bg-bg-primary border border-border-subtle p-2 pe-2 ps-4 rounded-2xl"
        >
      ...
      <orders-batch-actions
        v-if="selectedOrders.length > 0 && orders.length > 0"
