    ...
    <section class="bg-bg-primary border border-border-subtle rounded-2xl overflow-hidden shadow-2xl">
      ...
      <VTable :headers="headers" :items="zones" tableClass="w-full text-start">
        <template #cell(name)="{ item }">
          <span class="text-sm font-bold group-hover:text-tx-primary transition-colors">{{ item.name }}</span>
        </template>
        ...
        <template #cell(regions)="{ item }">
          <div class="flex items-center gap-1.5 flex-wrap">
            ...
            <span v-if="item.extraCount" class="text-[9px] font-bold text-tx-muted">+{{ item.extraCount }}</span>
          </div>
        </template>
        ...
        <template #cell(flatPrice)="{ item }">
          <div class="flex items-center justify-end gap-2">
            ...
            <span class="text-sm font-bold text-tx-secondary">{{ item.flatPrice }}</span>
          </div>
        </template>

        <template #cell(freeAbove)="{ item }">
          <div class="flex items-center justify-end gap-2">
            <span v-if="item.freeAbove !== 'NOT AVAILABLE'" class="text-[10px] font-bold text-tx-muted">$</span>
            <span
              :class="[item.freeAbove === 'NOT AVAILABLE' ? 'italic text-tx-muted text-[10px]' : 'text-sm font-bold text-tx-secondary']">
              {{ item.freeAbove }}
            </span>
          </div>
        </template>
