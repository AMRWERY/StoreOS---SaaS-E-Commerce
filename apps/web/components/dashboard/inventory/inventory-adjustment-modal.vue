    ...
    <Transition enter-active-class="transform transition ease-in-out duration-300" enter-from-class="translate-x-full"
      ...
      <div v-if="modelValue"
        ...
        <div class="flex-1 overflow-y-auto px-8 py-6 space-y-8">
          ...
          <div v-if="selectedItem"
            ...
            <div class="min-w-0 flex-1">
              <p class="text-[10px] text-tx-secondary mb-0.5">Selected Item</p>
              <h3 class="font-bold text-base text-tx-primary truncate">{{ selectedItem.name }}</h3>
              <p class="text-[10px] text-tx-secondary mt-1">Current: {{ selectedItem.current }} units</p>
            </div>
          </div>

          <!-- Adjustment Type -->
          <div class="space-y-3">
            <label class="text-[10px] font-bold text-tx-secondary tracking-widest">Adjustment Type</label>
          ...
          <!-- Quantity -->
          <div class="space-y-3">
            <label class="text-[10px] font-bold text-tx-secondary tracking-widest">Quantity</label>
          ...
          <!-- Reason -->
          <div class="space-y-3">
            <label class="text-[10px] font-bold text-tx-secondary tracking-widest">Reason For Change</label>
            <div class="relative">
          ...
          <!-- Notes -->
          <div class="space-y-3">
            <label class="text-[10px] font-bold text-tx-secondary tracking-widest">Notes (Optional)</label>
        ...
        <div class="p-6 border-t border-border-subtle flex items-center gap-3 bg-bg-elevated">
          <button @click="$emit('update:modelValue', false)"
            class="flex-1 py-3.5 rounded-xl font-bold text-sm bg-transparent border border-border-default text-tx-primary hover:bg-bg-elevated transition-colors">Cancel</button>
          <button @click="save"
            class="flex-1 py-3.5 rounded-xl font-bold text-sm bg-brand text-tx-primary hover:bg-brand-hover transition-colors shadow-lg shadow-brand/20">Save</button>
        </div>
      </div>
    </Transition>
  </div>
