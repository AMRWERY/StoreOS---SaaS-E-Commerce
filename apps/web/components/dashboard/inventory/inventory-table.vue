    ...
    >
      ...
      >
        <h2 class="font-bold flex items-center gap-2">
          <Icon name="ph:squares-four-bold" class="text-brand" />{{
            t("dashboard.inventory.inventoryList")
          }}
        </h2>
        <div class="flex items-center gap-4">
          <span class="text-[10px] font-bold text-tx-muted">{{
            t("dashboard.inventory.showing", { count: items.length })
          }}</span>
        </div>
      </div>
      ...
      <VTable :headers="headers" :items="paginatedItems">
        ...
        <template #cell(name)="{ item }">
          <div class="flex items-center gap-4">
            <div
              class="relative w-11 h-11 rounded-lg overflow-hidden border border-border-subtle bg-bg-primary"
            >
              <img
                :src="item.image"
            ...
            <div class="min-w-0">
              ...
              <p
                class="text-[10px] text-tx-secondary font-bold tracking-widest mt-0.5"
              >
                SKU: {{ item.sku }}
              </p>
            </div>
        ...
        <template #cell(category)="{ item }">
          <span
            class="text-[10px] font-black text-tx-secondary tracking-wider bg-bg-elevated px-2 py-1 rounded"
          >
            {{ item.category }}
          </span>
        </template>
        ...
        <template #cell(current)="{ item }">
          <div
            v-if="item.current === 0"
            class="text-[11px] font-black tracking-widest text-danger"
          >
            {{ t("dashboard.inventory.outOfStock") }}
          </div>
          <div
            v-else-if="item.current <= item.threshold"
            class="text-[11px] font-black tracking-widest text-accent"
          >
            {{ t("dashboard.inventory.low") }}: {{ item.current }}
          </div>
          <div
            v-else
            class="text-[11px] font-black tracking-widest text-success"
          >
            {{ item.current }}
          </div>
        </template>
        ...
        <template #cell(incoming)="{ item }">
          <span v-if="item.incoming > 0" class="text-sm font-bold text-brand"
            >+{{ item.incoming }}</span
          >
        ...
        <template #cell(threshold)="{ item }">
          <div
            class="flex items-center gap-2 group/edit cursor-pointer w-fit"
          ...
          >
            ...
            <template v-else>
              <span
                class="text-sm font-bold text-tx-secondary group-hover/edit:text-tx-primary transition-colors"
                >{{ item.threshold }}</span
              >
              <Icon
        ...
        <template #cell(lastUpdated)="{ item }">
          <span class="text-[10px] font-bold text-tx-muted tracking-wider">{{
            item.lastUpdated
          }}</span>
        </template>
...
const headers = computed(() => [
  {
    label: t("dashboard.inventory.productDetails"),
    key: "name",
    align: "start",
  },
  { label: t("dashboard.inventory.category"), key: "category", align: "start" },
  {
    label: t("dashboard.inventory.currentStock"),
    key: "current",
    align: "start",
  },
  { label: t("dashboard.inventory.incoming"), key: "incoming", align: "start" },
  {
    label: t("dashboard.inventory.threshold"),
    key: "threshold",
    align: "start",
  },
  {
    label: t("dashboard.inventory.lastUpdated"),
    key: "lastUpdated",
    align: "start",
  },
  {
    label: t("dashboard.inventory.quickAdjust"),
    key: "quickAdjust",
    align: "center",
  },
