    ...
    >
      <LazyVTable :headers="headers" :items="paginatedCustomers">
        ...
        <template #cell(customer)="{ item }">
          <div class="flex items-center gap-4">
            ...
            <div class="min-w-0 text-start">
              ...
              <p
                class="text-[10px] text-tx-secondary font-bold mt-0.5 tracking-tight"
              >
                {{ item.phone }}
              </p>
              <p class="text-[9px] text-tx-muted font-medium truncate">
                {{ item.email }}
              </p>
            </div>
          </div>
        ...
        <template #cell(location)="{ item }">
          <p class="text-xs text-tx-secondary font-medium leading-relaxed">
            {{ item.location }}
          </p>
        </template>
        ...
        <template #cell(lastActive)="{ item }">
          <p class="text-[11px] text-tx-secondary font-medium">
            {{ item.lastActive }}
          </p>
        </template>
        ...
        <template #cell(tags)="{ item }">
          <div class="flex flex-wrap gap-1.5">
            <span
              ...
              :class="[
                tag.color,
                'text-[8px] font-black px-2 py-0.5 rounded tracking-widest',
              ]"
            >
              {{ tag.text }}
            </span>
