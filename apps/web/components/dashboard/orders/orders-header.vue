<template>
  <div>
    ...
    <div class="flex flex-wrap gap-4 mb-8">
      <VSearchInput wrapperClass="flex-1 min-w-[300px]" :placeholder="$t('dashboard.orders.searchPlaceholder')" />
        ...
        <div class="flex items-center gap-3">
          ...
          <VDropdownMenu align="end" width="w-48">
            ...
            <template #default="{ close }">
              <button
                v-for="s in [$t('dashboard.orders.all'), $t('dashboard.orders.new'), $t('dashboard.orders.confirmed'), $t('dashboard.orders.processing'), $t('dashboard.orders.shipped'), $t('dashboard.orders.delivered'), $t('dashboard.orders.cancelled'), $t('dashboard.orders.refunded')]"
          ...
          <VDropdownMenu align="end" width="w-48">
            ...
            <template #default="{ close }">
              <button v-for="p in [$t('dashboard.orders.all'), $t('dashboard.orders.paid'), $t('dashboard.orders.unpaid'), $t('dashboard.orders.cod')]" :key="p" @click="selectPayment(p, close)"
                class="w-full text-start px-4 py-2.5 rounded-xl text-xs font-bold transition-all hover:bg-bg-elevated"
          ...
          <VDropdownMenu align="end" width="w-56">
            ...
            <template #default="{ close }">
              <button v-for="d in [$t('dashboard.orders.today'), $t('dashboard.orders.yesterday'), $t('dashboard.orders.last7Days'), $t('dashboard.orders.last30Days'), $t('dashboard.orders.thisMonth'), $t('dashboard.orders.customRange')]"
...
onMounted(() => { selectedDate.value = t('dashboard.orders.dateRange') })
