  ...
  <div class="h-full">
    ...
    >
      <div class="flex justify-between items-center mb-8">
        ...
        <Icon
          name="ph:dots-three-outline-fill"
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <h4
              class="text-sm font-bold truncate group-hover:text-brand transition-colors"
            >
              {{ prod.name }}
            </h4>
            <p
              class="text-[10px] text-tx-muted font-bold mt-0.5 tracking-tight"
            >
              {{ prod.soldToday }} {{ t("dashboard.home.soldToday") }}
            </p>
          </div>
          <span
            class="text-xs font-black text-tx-muted group-hover:text-brand/50 transition-colors"
            >#0{{ i + 1 }}</span
          >
        </div>
      </div>
