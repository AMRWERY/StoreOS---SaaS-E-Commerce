      ...
      <div class="space-y-12">
        <div v-for="category in categories" :key="category.label">
          <div class="flex items-center gap-4 mb-6">
            <div :class="['w-8 h-8 rounded-lg flex items-center justify-center', category.iconBg]">
              <Icon :name="category.icon" :class="['text-base', category.iconColor]" />
            </div>
            <h3 class="text-xs font-black tracking-[0.2em] text-tx-muted">{{ category.label }}</h3>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            ...
            >
              ...
              <div class="text-center">
                <div class="text-xs font-bold text-tx-primary">{{ item.name }}</div>
                <div class="text-[10px] text-tx-muted mt-0.5">{{ item.sub }}</div>
              </div>
      ...
      <div class="mt-24 text-center">
        ...
        <div class="grid md:grid-cols-3 gap-8 relative">
          ...
          <div v-for="step in steps" :key="step.id" class="flex flex-col items-center group">
            <div class="w-20 h-20 rounded-2xl bg-brand-dim border border-brand/20 flex items-center justify-center text-2xl font-black text-brand mb-6 group-hover:scale-105 transition-transform shadow-lg shadow-brand/10">
              {{ step.id }}
            </div>
            <h3 class="text-base font-bold mb-3">{{ step.title }}</h3>
            <p class="text-tx-secondary text-sm leading-relaxed max-w-xs">{{ step.desc }}</p>
          </div>
        </div>
      </div>
...
const categories = computed(() => [
  {
    ...
    items: [
      { name: 'Stripe', sub: 'Cards & Wallets', icon: 'ph:stripe-logo-fill', bg: 'bg-[#635bff]/10', color: 'text-[#635bff]', live: true },
      { name: 'Paymob', sub: 'Local Gateway', icon: 'ph:bank-fill', bg: 'bg-emerald-500/10', color: 'text-emerald-400', live: true },
      { name: 'Vodafone Cash', sub: 'Mobile Wallet', icon: 'ph:device-mobile-fill', bg: 'bg-red-500/10', color: 'text-red-400', live: true },
      { name: 'Cash on Delivery', sub: 'COD', icon: 'ph:money-fill', bg: 'bg-yellow-500/10', color: 'text-yellow-400', live: true },
      { name: 'Fawry', sub: 'Egyptian Pay', icon: 'ph:qr-code-fill', bg: 'bg-orange-500/10', color: 'text-orange-400', soon: true },
      { name: 'PayPal', sub: 'Global Pay', icon: 'ph:paypal-logo-fill', bg: 'bg-sky-500/10', color: 'text-sky-400', soon: true },
    ]
  },
  {
    ...
    items: [
      { name: 'Aramex', sub: 'Express', icon: 'ph:package-fill', bg: 'bg-orange-500/10', color: 'text-orange-400', live: true },
      { name: 'J&T Express', sub: 'Last Mile', icon: 'ph:lightning-fill', bg: 'bg-red-500/10', color: 'text-red-400', live: true },
      { name: 'Bosta', sub: 'Same Day', icon: 'ph:motorcycle-fill', bg: 'bg-brand-dim', color: 'text-brand', live: true },
      { name: 'DHL', sub: 'International', icon: 'ph:airplane-tilt-fill', bg: 'bg-yellow-500/10', color: 'text-yellow-400', live: true },
      { name: 'Mylerz', sub: 'Egypt Delivery', icon: 'ph:map-pin-fill', bg: 'bg-emerald-500/10', color: 'text-emerald-400', soon: true },
      { name: 'FedEx', sub: 'Global', icon: 'ph:globe-fill', bg: 'bg-purple-500/10', color: 'text-purple-400', soon: true },
    ]
  },
  {
    ...
    items: [
      { name: 'WhatsApp', sub: 'Order Updates', icon: 'ph:whatsapp-logo-fill', bg: 'bg-emerald-500/10', color: 'text-emerald-400', live: true },
      { name: 'Email', sub: 'Transactional', icon: 'ph:envelope-fill', bg: 'bg-brand-dim', color: 'text-brand', live: true },
      { name: 'SMS', sub: 'Local & Intl', icon: 'ph:chat-fill', bg: 'bg-info-dim', color: 'text-info', live: true },
      { name: 'Slack', sub: 'Team Alerts', icon: 'ph:slack-logo-fill', bg: 'bg-purple-500/10', color: 'text-purple-400', soon: true },
    ]
  },
  {
    ...
    iconColor: 'text-info',
    items: [
      { name: 'Meta Pixel', sub: 'Facebook & IG', icon: 'ph:meta-logo-fill', bg: 'bg-brand-dim', color: 'text-brand', live: true },
      { name: 'TikTok Pixel', sub: 'TikTok Ads', icon: 'ph:tiktok-logo-fill', bg: 'bg-[#ff0050]/10', color: 'text-[#ff0050]', live: true },
      { name: 'Google Analytics', sub: 'GA4', icon: 'ph:google-logo-fill', bg: 'bg-yellow-500/10', color: 'text-yellow-400', live: true },
