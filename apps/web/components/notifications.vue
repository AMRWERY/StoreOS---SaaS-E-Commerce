<template>
  <div>
    ...
    <Transition name="fade">
      <div v-if="modelValue" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100]" @click="$emit('close')"></div>
    </Transition>
    ...
    >
      ...
      <div class="p-8 border-b border-white/5 bg-white/[0.01]">
        ...
        <h2 class="text-2xl font-bold text-white tracking-tight flex items-center gap-3">
           Updates <span class="bg-indigo-500/10 text-indigo-500 text-[10px] px-2 py-0.5 rounded-full border border-indigo-500/20">{{ unreadCount }} New</span>
        </h2>
      </div>
      ...
      <div class="flex border-b border-white/5 bg-black/20">
        ...
        >
          {{ tab.label }}
          <div v-if="activeTab === tab.id" class="absolute bottom-0 start-0 end-0 h-0.5 bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar bg-black/5">
          ...
          >
            <div class="flex gap-5">
              ...
              <div class="relative shrink-0">
                <div :class="['w-14 h-14 rounded-2xl flex items-center justify-center border border-white/5 shadow-inner', n.bgColor]">
                  <ClientOnly>
                    <icon :name="n.icon" :class="['text-2xl', n.iconColor]" />
                  </ClientOnly>
                </div>
                <div v-if="!n.read" class="absolute -top-1.5 -end-1.5 w-4 h-4 bg-indigo-500 rounded-full border-2 border-[#080809]"></div>
              </div>
              ...
              <div class="min-w-0 flex-1">
                ...
                <h4 class="text-sm font-bold text-white group-hover:text-indigo-400 transition-colors">{{ n.title }}</h4>
                <p class="text-xs text-gray-500 mt-2 line-clamp-2 leading-relaxed font-medium">{{ n.message }}</p>
                
                <div v-if="n.type === 'orders'" class="mt-4">
                  <button class="text-[10px] font-black tracking-widest text-indigo-400 hover:text-white flex items-center gap-2 transition-all uppercase">
                    View Order Details 
                    <ClientOnly>
        ...
        <div v-else class="h-full flex flex-col items-center justify-center text-center p-12">
          <div class="w-24 h-24 rounded-[2rem] bg-white/5 flex items-center justify-center mb-8 border border-white/5">
            <ClientOnly>
          ...
          <p class="text-sm text-gray-600 mt-3 max-w-[260px] leading-relaxed">Notifications about your store activity and security will appear here.</p>
        </div>
      </div>

      <!-- Footer -->
      <div class="p-8 border-t border-white/5 bg-[#0c0c0e]">
        <button class="w-full py-4 rounded-2xl font-black text-[10px] tracking-widest uppercase bg-white/5 hover:bg-white/10 text-white transition-all border border-white/5 hover:border-white/10">
