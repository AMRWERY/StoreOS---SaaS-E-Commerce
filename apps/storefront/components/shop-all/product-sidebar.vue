<template>
  <aside class="w-full lg:w-[240px] shrink-0 space-y-12">
    <div>
      ...
      <p class="text-tx-muted text-[12px] font-semibold">/ {{ totalItems }} {{ $t('storefront.shop.items') }}</p>
    </div>

    <!-- Category -->
    <div class="space-y-4">
      <h4 class="text-[11px] font-black tracking-[0.2em] text-tx-secondary">{{ $t('storefront.shop.category') }}</h4>
      <div class="space-y-3">
    ...
    <!-- Price Range -->
    <div class="space-y-6">
      <h4 class="text-[11px] font-black tracking-[0.2em] text-tx-secondary">{{ $t('storefront.shop.priceRange') }}</h4>
      <div class="relative h-1 w-full bg-border-subtle rounded-full">
        <div class="absolute h-full bg-brand start-0 end-[20%]"></div>
        <div class="absolute -top-1.5 start-0 w-4 h-4 bg-white rounded-full border-2 border-brand cursor-pointer">
        </div>
        <div
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="bg-bg-primary border border-border-subtle rounded-[8px] p-2 px-3">
          <p class="text-[8px] font-black text-tx-muted mb-1">{{ $t('storefront.shop.min') }} ($)</p>
          <p class="text-[12px] font-semibold text-tx-secondary">{{ modelValue.priceMin }}</p>
        </div>
        <div class="bg-bg-primary border border-border-subtle rounded-[8px] p-2 px-3">
          <p class="text-[8px] font-black text-tx-muted mb-1">{{ $t('storefront.shop.max') }} ($)</p>
          <p class="text-[12px] font-semibold text-tx-primary">{{ modelValue.priceMax }}+</p>
        </div>
      </div>
    </div>
    ...
    <!-- Colors -->
    <div class="space-y-4">
      <h4 class="text-[11px] font-black tracking-[0.2em] text-tx-secondary">{{ $t('storefront.shop.colors') }}</h4>
      <div class="flex flex-wrap gap-3">
        <div v-for="color in colors" :key="color.name"
          ...
          :class="{ 'border-brand ring-2 ring-brand/20': color.name === modelValue.selectedColor }">
    ...
    <!-- Sizes -->
    <div class="space-y-4">
      <h4 class="text-[11px] font-black tracking-[0.2em] text-tx-secondary">{{ $t('storefront.shop.sizes') }}</h4>
      <div class="flex flex-wrap gap-2">
        <button v-for="size in sizes" :key="size" @click="modelValue.selectedSize = size"
          ...
          :class="modelValue.selectedSize === size ? 'bg-brand border-brand text-white' : 'bg-bg-primary border-border-subtle text-tx-muted hover:border-tx-secondary'">
          {{ size }}
        </button>
      </div>
    </div>
