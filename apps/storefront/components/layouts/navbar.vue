<template>
  <div>
    <nav
      class="fixed top-0 w-full z-50 border-b border-border-subtle bg-bg-base/80 backdrop-blur-md"
    >
      <div
        class="max-w-7xl mx-auto px-12 md:px-24 h-16 flex items-center justify-between"
      >
          ...
          <div class="hidden md:flex items-center gap-10 h-full">
            ...
            >
              ...
              >
                <span
                  :class="[
                    isActive || isNavLinkActive(link)
                      ? 'text-brand'
                      : 'text-tx-secondary group-hover:text-brand',
                  ]"
                ...
                <div
                  v-if="isActive || isNavLinkActive(link)"
...
const navLinks = computed<StorefrontNavLink[]>(() => [
  ...
  {
    label: t("storefront.nav.storeBuilder"),
    path: "/dashboard/builder/home",
    match: "exact",
  },
]);
</script>