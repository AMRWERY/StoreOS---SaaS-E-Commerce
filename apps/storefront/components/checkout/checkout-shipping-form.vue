    ...
    <section class="space-y-8">
      ...
      <div class="grid grid-cols-2 gap-4">
        <VInput v-model="form.city" :label="$t('storefront.checkout.city')"
          ...
          inputClass="!w-full !bg-bg-primary !border-border-subtle !rounded-[8px] !px-5 !py-4 !text-[13px] focus:!border-brand !outline-none !transition-all" />
        <div class="space-y-2">
          <label class="text-[10px] font-black tracking-[0.2em] text-tx-muted">{{ $t('storefront.checkout.governorate') }}</label>
          <div class="relative">
      ...
      <div class="space-y-3">
        <div v-for="method in shippingMethods" :key="method.id"
          ...
          :class="form.selectedMethod === method.id
            ...
            : 'bg-transparent border-border-subtle hover:border-border-default'" @click="form.selectedMethod = method.id">
          <div class="flex items-center gap-5">
            <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all"
              :class="form.selectedMethod === method.id ? 'border-brand' : 'border-border-default'">
              <div v-if="form.selectedMethod === method.id" class="w-2.5 h-2.5 rounded-full bg-brand" />
            </div>
            <div>
              <p class="text-[14px] font-bold">{{ method.name }}</p>
              <p class="text-[11px] text-tx-muted font-medium">{{ method.time }}</p>
            </div>
          </div>
          <p class="text-sm font-bold text-tx-secondary">EGP {{ method.price }}</p>
        </div>
      </div>
    </section>
