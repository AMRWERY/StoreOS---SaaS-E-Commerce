  ...
  <div class="flex-1 min-w-0 space-y-6">
    ...
    <section class="bg-[#0D0D18] border border-[#1C1C30] rounded-[16px] p-8">
      <div class="flex flex-col sm:flex-row justify-between gap-6 mb-12">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-[#50506A] mb-3">Order Identification</p>
          <div class="flex items-center gap-3">
            <h2 class="font-mono text-2xl font-bold tracking-tighter">{{ orderId }}</h2>
            <button
              title="Copy order ID"
              class="text-[#50506A] hover:text-[#6366F1] transition-colors"
              @click="copyOrderId"
            >
        ...
        <div class="sm:text-end">
          <p class="text-[10px] font-black uppercase tracking-[0.2em] text-[#50506A] mb-3">Estimated Delivery</p>
          <p class="font-mono text-lg font-bold text-[#F0F0FF]">{{ estimatedDelivery }}</p>
        </div>
      </div>
      ...
      <div class="relative pt-10 pb-4">
        ...
        <div class="relative flex justify-between">
          <div v-for="step in trackerSteps" :key="step.label" class="flex flex-col items-center">
            ...
            />
            <span
              class="mt-4 text-[9px] font-black uppercase tracking-widest"
    ...
    <section class="bg-[#0D0D18] border border-[#1C1C30] rounded-[16px] p-8">
      <div class="flex items-center justify-between mb-8">
        ...
        <span class="font-mono text-[11px] font-bold text-[#50506A] uppercase tracking-widest">
          {{ items.length }} Items
        </span>
      </div>
      ...
      <div class="space-y-6">
        <div v-for="item in items" :key="item.id" class="flex items-center gap-5 group">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[11px] text-[#50506A] mt-1 font-medium uppercase tracking-tighter truncate">{{ item.meta }}</p>
          </div>
          <div class="text-end shrink-0">
            <p class="font-mono text-sm font-bold text-[#F0F0FF] tracking-tighter">
              <span class="text-[11px] text-[#50506A] me-1">$</span>{{ item.price }}
            </p>
            <p class="font-mono text-[10px] font-black text-[#50506A] uppercase mt-1">Qty: {{ item.qty }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-end gap-5 pt-2">
      <nuxt-link-locale
        to="/shop-all"
        class="text-[11px] font-bold text-[#8888AA] hover:text-[#F0F0FF] transition-colors uppercase tracking-[0.2em] flex items-center gap-2 group"
      >
        Continue Shopping
