<template>
  <div class="flex-1 min-w-0 space-y-6">
    <!-- Order ID & Tracker -->
    <section
      class="bg-bg-primary border border-border-subtle rounded-[16px] p-8"
    >
      <div class="flex flex-col sm:flex-row justify-between gap-6 mb-12">
        <div>
          <p class="text-[10px] font-black tracking-[0.2em] text-tx-muted mb-3">
            {{ t("storefront.checkout.orderIdentification") }}
          </p>
        ...
        <div class="sm:text-end">
          <p class="text-[10px] font-black tracking-[0.2em] text-tx-muted mb-3">
            {{ t("storefront.checkout.estimatedDelivery") }}
          </p>
          <p class="text-lg font-bold text-tx-primary">
            {{ estimatedDelivery }}
          </p>
        </div>
      </div>
      ...
      <div class="relative pt-10 pb-4">
        <div
          class="absolute top-[47px] start-0 w-full h-[2px] bg-border-subtle"
        />
        <div
          class="absolute top-[47px] start-0 w-1/4 h-[2px] bg-brand shadow-[0_0_10px_rgba(99,102,241,0.5)]"
        />
          ...
          >
            <div
              ...
              :class="{
                ...
                'bg-white border-brand shadow-[0_0_15px_#fff]':
                  step.status === 'active',
                'bg-bg-primary border-border-subtle': step.status === 'pending',
              }"
            />
            <span
              class="mt-4 text-[9px] font-black tracking-widest"
              :class="
                step.status === 'pending'
                  ? 'text-tx-muted'
                  : 'text-tx-secondary'
    ...
    >
      <div class="flex items-center justify-between mb-8">
        <h3 class="text-base font-bold tracking-tight">
          {{ t("storefront.checkout.shipmentDetails") }}
        </h3>
      ...
      <div class="space-y-6">
        ...
        >
          <div
            class="w-16 h-16 rounded-[10px] overflow-hidden bg-black border border-border-subtle shrink-0"
          >
            <img
              :src="item.img"
              :alt="item.name"
          ...
          <div class="flex-1 min-w-0">
            <h4
              class="text-[14px] font-bold text-tx-primary group-hover:text-brand transition-colors truncate"
            >
              {{ item.name }}
            </h4>
            <p
              class="text-[11px] text-tx-muted mt-1 font-medium tracking-tighter truncate"
            >
              {{ item.meta }}
            </p>
          </div>
          <div class="text-end shrink-0">
            ...
            <p class="text-[10px] font-black text-tx-muted mt-1">
