<template>
  <aside class="space-y-6">
    <div class="bg-bg-primary border border-border-subtle rounded-[12px] p-8 shadow-2xl">
      ...
      <div class="space-y-4 border-b border-border-subtle pb-8">
        <div class="flex justify-between text-[13px] font-medium text-tx-secondary">
        ...
        <div class="flex justify-between text-[13px] font-medium text-tx-secondary">
          ...
          <span class="text-[11px] italic text-tx-muted">{{ $t('storefront.checkout.calculatedNextStep') }}</span>
        </div>
        <div class="flex justify-between text-[13px] font-medium text-tx-secondary">
      ...
      <div class="mt-8 space-y-3">
        <p class="text-[10px] font-black text-tx-muted tracking-widest">{{ $t('storefront.checkout.discountCode') }}</p>
      ...
      <div class="mt-10 mb-8 flex items-end justify-between">
        <div class="space-y-1">
          <p class="text-[9px] font-black text-tx-muted tracking-[0.2em]">{{ $t('storefront.checkout.estimatedTotal') }}</p>
          <h3 class="text-[12px] font-black tracking-widest text-tx-secondary">{{ $t('storefront.checkout.total') }}</h3>
        </div>
      ...
      <p class="text-center text-[8px] font-black text-tx-muted tracking-[0.2em] mt-6">
        {{ $t('storefront.checkout.taxesAtFinalStep') }}
      </p>
    </div>

    <!-- Priority Alert -->
    <div class="bg-indigo-500/5 border border-brand/20 rounded-[12px] p-6 flex items-start gap-4">
