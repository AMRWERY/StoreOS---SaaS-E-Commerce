<template>
  <main class="space-y-6">
    <div class="flex items-baseline justify-between">
      ...
      <span class="text-tx-muted text-[12px] font-semibold tracking-widest">
        {{ items.length }} Items
      </span>
    </div>

    <div class="bg-bg-primary border border-border-subtle rounded-[12px] divide-y divide-border-subtle overflow-hidden">
      <div v-for="item in items" :key="item.id"
        class="p-5 flex items-center gap-4 group hover:bg-bg-elevated transition-colors">
        <!-- Product Image -->
        <div class="w-20 h-20 rounded-[8px] overflow-hidden bg-black border border-border-subtle shrink-0">
          <img :src="item.image"
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-start justify-between gap-2 mb-2">
            <div class="min-w-0">
              <h3
                class="text-[14px] font-bold text-white group-hover:text-brand transition-colors leading-tight truncate">
                {{ item.name }}
              </h3>
              <p class="text-[10px] text-tx-muted mt-0.5 font-semibold tracking-tighter truncate">
                {{ item.meta }}
              </p>
            </div>
            <p class="text-brand font-bold text-[15px] tracking-tighter shrink-0">
    ...
    <div class="flex flex-wrap gap-8 pt-2 opacity-40 hover:opacity-100 transition-opacity">
      <div v-for="trust in trustBadges" :key="trust.text" class="flex items-center gap-2">
        <Icon :name="trust.icon" class="w-4 h-4 text-tx-secondary" />
        <span class="text-[9px] font-black tracking-[0.2em] text-tx-secondary">{{ trust.text }}</span>
      </div>
    </div>
  </main>
</template>
