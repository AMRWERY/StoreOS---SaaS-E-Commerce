  ...
  <aside class="lg:col-span-3 space-y-6">
    <div class="bg-bg-primary border border-border-subtle rounded-[16px] p-10">
      ...
      <div class="flex items-center justify-between mb-10">
        ...
        <div class="flex items-center gap-2 bg-brand/10 px-2 py-1 rounded-[6px]">
      ...
      <div class="space-y-6 mb-10 border-b border-border-subtle pb-10">
        <div v-for="item in items" :key="item.id" class="flex items-center gap-5 group">
          <div class="w-16 h-16 rounded-[8px] overflow-hidden bg-black border border-border-subtle shrink-0">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-tx-muted mt-1 font-medium tracking-tighter">{{ item.meta }}</p>
            <p class="text-[9px] font-bold text-brand mt-1 tracking-widest">{{ $t('storefront.checkout.qty') }}: {{ item.qty }}</p>
          </div>
          <p class="text-[13px] font-bold text-tx-secondary shrink-0">EGP {{ item.price }}</p>
        </div>
      </div>
      ...
      <div class="pt-8 border-t border-border-subtle space-y-2">
        <div class="flex items-baseline justify-between">
          <span class="text-[14px] font-bold tracking-widest text-tx-primary">{{ $t('storefront.checkout.totalDue') }}</span>
          <div class="text-end">
            <p class="text-[32px] font-bold text-brand tracking-tighter leading-none">
              <span class="text-[14px] me-1 opacity-60">EGP</span>{{ total }}
            </p>
          </div>
        </div>
        <p class="text-end text-[9px] font-black text-tx-muted tracking-[0.2em]">{{ $t('storefront.checkout.allTaxesIncluded') }}</p>
      </div>
    </div>

    <!-- Security Badge -->
    <div class="bg-black/20 border border-border-subtle rounded-[12px] p-4 px-6 flex items-center gap-4">
