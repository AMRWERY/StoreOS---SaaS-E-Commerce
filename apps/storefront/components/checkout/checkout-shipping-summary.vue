<template>
  <aside class="lg:col-span-3 space-y-6">
    <div class="bg-[#0D0D18] border border-[#1C1C30] rounded-[16px] p-10">
      ...
      <div class="flex items-center justify-between mb-10">
        ...
        <div class="flex items-center gap-2 bg-[#6366F1]/10 px-2 py-1 rounded-[6px]">
          ...
          <span class="text-[9px] font-black uppercase text-[#6366F1] tracking-widest">
            {{ items.length }} Items
          </span>
        </div>
      </div>
      ...
      <div class="space-y-6 mb-10 border-b border-[#1C1C30] pb-10">
        <div v-for="item in items" :key="item.id" class="flex items-center gap-5 group">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[10px] text-[#50506A] mt-1 font-medium uppercase tracking-tighter">{{ item.meta }}</p>
            <p class="text-[9px] font-bold text-[#6366F1] mt-1 uppercase tracking-widest">Qty: {{ item.qty }}</p>
          </div>
      ...
      <div class="pt-8 border-t border-[#1C1C30] space-y-2">
        <div class="flex items-baseline justify-between">
          <span class="text-[14px] font-bold uppercase tracking-widest text-[#F0F0FF]">Total Due</span>
          <div class="text-end">
        ...
        <p class="text-end text-[9px] font-black uppercase text-[#50506A] tracking-[0.2em]">All Taxes Included</p>
      </div>
    </div>
    ...
    <div class="bg-black/20 border border-[#1C1C30] rounded-[12px] p-4 px-6 flex items-center gap-4">
      ...
      <p class="text-[11px] font-bold text-[#50506A] uppercase tracking-widest">
