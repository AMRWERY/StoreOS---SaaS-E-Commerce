    ...
    <Teleport to="body">
      ...
      <Transition name="slide">
        ...
        >
          ...
          <div class="px-6 py-6 border-b border-border-subtle bg-bg-primary">
            <div class="flex items-center justify-between">
              <div>
                ...
                <h2 class="text-2xl font-bold text-tx-primary tracking-tight">
                  {{ t("storefront.bag.shoppingBag") }}
                </h2>
              </div>
              <button
                @click="isInternalOpen = false"
                class="text-tx-muted hover:text-tx-primary transition-colors p-2"
              >
          ...
          <!-- Cart Items List -->
          <div
            class="flex-1 overflow-y-auto hide-scrollbar bg-bg-primary p-6 space-y-6"
          >
                ...
                <div class="flex-1 flex flex-col justify-between py-1">
                  <div>
                    <div class="flex justify-between items-start gap-2">
                      ...
                      <button
                        ...
                        class="text-tx-muted hover:text-red-500 transition-colors"
                      >
                        <Icon name="ph:trash-bold" class="text-sm" />
                      </button>
                    </div>
                    <p class="text-[10px] text-tx-muted tracking-tighter mt-1">
                      {{ item.category }}
                    </p>
                  </div>
                  ...
                  <div class="flex items-center justify-between mt-4">
                    ...
                    >
                      ...
                      <span
                        class="px-2 text-xs font-bold text-tx-primary min-w-[20px] text-center"
                        >{{ item.quantity }}</span
                      >
                      <button
                        @click="updateQuantity(item.id, 1)"
            ...
            >
              ...
              >
                <Icon
                  name="ph:shopping-bag-open-bold"
                  class="text-3xl text-tx-muted opacity-40"
                />
              </div>
              <h4 class="text-xl font-bold text-tx-secondary tracking-tight">
                {{ t("storefront.bag.emptyTitle") }}
              </h4>
              <p
                class="text-sm text-tx-muted mt-2 max-w-[240px] leading-relaxed"
              >
          ...
          <div
            ...
            class="p-6 border-t border-border-subtle bg-bg-base"
          >
            <div class="space-y-3 mb-6">
              ...
              <div class="flex justify-between text-xs text-tx-muted">
                ...
                <span
                  class="text-emerald-500 font-bold tracking-widest text-[9px]"
                  >{{ t("storefront.bag.calculatedAtCheckout") }}</span
                >
              </div>
              <div
                class="pt-3 border-t border-border-subtle/50 flex justify-between items-end"
              >
                <span class="text-sm font-bold text-tx-primary">{{
                  t("storefront.bag.totalAmount")
                }}</span>
