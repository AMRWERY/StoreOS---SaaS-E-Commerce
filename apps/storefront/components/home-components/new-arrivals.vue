      ...
      <div class="max-w-7xl mx-auto px-12 md:px-24">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <div v-for="product in products" :key="product.id"
            class="group relative bg-bg-base border border-border-subtle rounded-[16px] p-2 transition-all hover:border-brand/30 hover:shadow-xl hover:shadow-brand/5 flex flex-col h-full">
            <div class="relative overflow-hidden rounded-[12px] h-[200px] bg-bg-elevated">
              ...
              <span v-if="product.tag" :class="[product.tag.type === 'new' ? 'bg-brand' : 'bg-red-500']"
            ...
            <div class="p-3 flex-1 flex flex-col">
              ...
              <div class="mt-2 flex items-baseline gap-2 font-mono font-bold">
                <span class="text-tx-primary text-[16px]">EGP {{ product.price.toLocaleString() }}</span>
                <span v-if="product.oldPrice" class="text-tx-muted opacity-60 line-through text-[11px]">EGP {{
