<template>
  <div>
    <!-- Trigger Button -->
    <button @click="isInternalOpen = true" :class="[
        ...
        <aside v-if="isInternalOpen"
          ...
          <div class="px-6 py-6 border-b border-border-subtle bg-bg-primary">
            ...
            <h2 class="text-2xl font-bold text-tx-primary tracking-tight flex items-center gap-3">
              ...
              <span class="bg-brand-dim text-brand text-[10px] px-2 py-0.5 rounded-full border border-brand/20">{{
          ...
          <div class="flex border-b border-border-subtle bg-bg-base">
            <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="[
              ...
              <div v-if="activeTab === tab.id"
                class="absolute bottom-0 start-0 end-0 h-0.5 bg-brand shadow-[0_0_10px_rgba(99,102,241,0.5)]"></div>
            </button>
          </div>
          ...
          <div class="flex-1 overflow-y-auto hide-scrollbar bg-bg-primary">
              ...
              <div v-for="n in filteredNotifications" :key="n.id"
                ...
                <div class="flex gap-4">
                  ...
                  <div class="relative shrink-0">
                    ...
                    <span v-if="!n.read"
                      class="absolute -top-1 -end-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-bg-base animate-ping"></span>
                  </div>

                  <!-- Details -->
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center justify-between gap-2 mb-1">
                      <p class="text-[9px] font-black tracking-widest text-tx-muted">
                        {{ n.category }}
                      </p>
                      <span class="text-[9px] font-bold text-tx-muted tracking-tighter">{{ n.time }}</span>
                    </div>
                    <h4 class="text-sm font-bold text-tx-primary group-hover:text-brand transition-colors">
                      {{ n.title }}
                    </h4>
                    <p class="text-xs text-tx-secondary mt-1 line-clamp-2 leading-relaxed font-medium">
                      {{ n.message }}
                    </p>
            ...
            <div v-else class="h-full flex flex-col items-center justify-center text-center p-8">
              ...
              <p class="text-sm text-tx-muted mt-2 max-w-[260px] leading-relaxed">
          ...
          <div class="px-6 py-6 border-t border-border-subtle bg-bg-base">
            <LazyVButton variant="secondary" className="w-full py-4 font-black">
