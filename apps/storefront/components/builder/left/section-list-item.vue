<template>
  <component :is="UiButton" variant="none" type="button" :class-name="rowClass" @click="onSelect">
    <Icon name="ph:dots-six-vertical-bold" class="text-tx-muted opacity-0 transition-opacity group-hover:opacity-100" />
    <span class="min-w-0 flex-1 truncate text-[12px] font-medium">{{ label }}</span>
    <Icon v-if="section.is_hidden" name="ph:eye-slash-bold" class="text-tx-muted" />
