<template>
  <div
    class="group relative cursor-pointer outline-none transition-opacity"
    :class="section.is_hidden ? 'opacity-[0.45]' : ''"
    role="button"
    tabindex="0"
  ...
  >
    <SectionOverlay :selected="selected" :highlighted="!selected && isHovered" />

    <div
      v-show="selected || isHovered"
      class="absolute start-0 top-0 z-20 rounded-br-md bg-brand/90 px-2 py-0.5 text-[9px] font-bold tracking-widest text-white"
    >
      {{ label }}
    </div>
      ...
      >
        <div class="max-w-xl ps-16">
          <h2 class="text-3xl font-bold text-white">
            {{ str('headline', 'Hero Slideshow') }}
          </h2>
          <p class="mt-4 text-white/80">
            {{ str('subheadline', 'Add multiple slides in the settings panel.') }}
          </p>
        </div>
        <div class="absolute bottom-5 start-0 end-0 flex justify-center gap-2">
          <span
            ...
            :class="i === 1 ? 'bg-white' : 'bg-white/35'"
          />
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'hero_video'">
      <div class="relative flex min-h-[380px] items-center justify-center bg-neutral-900">
        ...
        <div class="relative z-10 text-center">
          <h2 class="text-3xl font-bold text-white">
            {{ str('headline', 'Hero Video') }}
          </h2>
          <p class="mt-3 text-sm text-neutral-400">
            {{ str('subheadline', 'Add a video URL in the settings panel.') }}
          </p>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'products_grid'">
      <div class="border-t border-neutral-200 px-8 py-12">
        <h3 class="text-lg font-bold text-neutral-900">
          {{ str('heading', 'Our Products') }}
        </h3>

        <!-- shop style (default) — Image 4: category + stars + add button -->
        <div v-if="str('card_style','shop') !== 'sale' && str('card_style','shop') !== 'horizontal' && str('card_style','shop') !== 'editorial'" class="mt-8 grid grid-cols-3 gap-5">
            ...
            <div class="mt-3">
              ...
              <div class="mt-1 flex items-center gap-1">
                <div class="flex text-amber-400">
                  <Icon v-for="s in 4" :key="s" name="ph:star-fill" class="text-xs" />
                  <Icon name="ph:star-bold" class="text-xs text-neutral-300" />
                </div>
                <span class="text-[11px] text-neutral-400">({{ i + 5 }})</span>
              </div>
              <div class="mt-3 flex items-center justify-between gap-2">
                <div>
                  <span class="text-sm font-bold text-neutral-900">$80</span>
                  <span class="ms-1.5 text-xs text-neutral-400 line-through">$100</span>
                </div>
        ...
        <!-- sale style — Image 1: discount badge + wishlist + sale price -->
        <div v-else-if="str('card_style','shop') === 'sale'" class="mt-8 grid grid-cols-3 gap-5">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="relative overflow-hidden rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            ...
            <button type="button" class="absolute end-3 top-3 rounded-full border border-neutral-200 bg-white p-1.5 shadow-sm">
        ...
        <!-- horizontal style — Image 2: small image left, details right -->
        <div v-else-if="str('card_style','shop') === 'horizontal'" class="mt-6 space-y-3">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="flex items-center gap-4 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-neutral-400">3 variants · In stock</p>
            </div>
          </div>
        </div>

        <!-- editorial style — Image 3: image top + bold title + description + button -->
        <div v-else-if="str('card_style','shop') === 'editorial'" class="mt-8 grid grid-cols-3 gap-5">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
            ...
            <div class="p-4">
              ...
              <button type="button" class="mt-4 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white">
                View product
              </button>
            </div>
          </div>
    ...
    <template v-else-if="section.type === 'products_carousel'">
      <div class="overflow-hidden px-8 py-12">
        ...
        <!-- shop (default) -->
        <div v-if="str('card_style','shop') !== 'sale' && str('card_style','shop') !== 'horizontal' && str('card_style','shop') !== 'editorial'" class="flex gap-5 overflow-hidden">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="w-48 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
            ...
            <div class="mt-2">
              ...
              <div class="mt-1 flex text-amber-400">
                <Icon v-for="s in 4" :key="s" name="ph:star-fill" class="text-[10px]" />
                <Icon name="ph:star-bold" class="text-[10px] text-neutral-300" />
              </div>
              <div class="mt-2 flex items-center justify-between gap-1">
                <div>
                  <span class="text-xs font-bold text-neutral-900">$80</span>
                  <span class="ms-1 text-[10px] text-neutral-400 line-through">$100</span>
                </div>
                <button type="button" class="flex items-center gap-1 rounded-lg border border-neutral-200 bg-neutral-50 px-2 py-1 text-[10px] font-semibold text-neutral-700">
        ...
        <!-- sale -->
        <div v-else-if="str('card_style','shop') === 'sale'" class="flex gap-5 overflow-hidden">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="relative w-48 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
            <span class="absolute start-2.5 top-2.5 rounded-full bg-green-400 px-2 py-0.5 text-[10px] font-bold text-white">20% off</span>
            <button type="button" class="absolute end-2.5 top-2.5 rounded-full border border-neutral-100 bg-white p-1">
              <Icon name="ph:bookmark-bold" class="text-[11px] text-neutral-400" />
            </button>
            <div class="flex aspect-square items-center justify-center rounded-xl bg-neutral-50">
              <div class="h-24 w-full bg-contain bg-center bg-no-repeat" :style="{ backgroundImage: `url('${img}')` }" />
            </div>
            <div class="mt-2 space-y-1">
              <p class="text-xs font-medium text-neutral-900">Product {{ i + 1 }}</p>
              <div class="flex items-center gap-1.5">
                <span class="text-xs font-bold text-neutral-900">$29</span>
                <span class="text-[10px] text-neutral-400 line-through">$59</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- horizontal -->
        <div v-else-if="str('card_style','shop') === 'horizontal'" class="w-full space-y-2">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="flex items-center gap-3 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm">
            <div class="size-14 shrink-0 overflow-hidden rounded-xl bg-neutral-50">
              <div class="h-full w-full bg-contain bg-center bg-no-repeat" :style="{ backgroundImage: `url('${img}')` }" />
            </div>
            <div class="min-w-0 flex-1">
              <p class="text-xs font-semibold text-neutral-900">Product {{ i + 1 }}</p>
              <p class="text-[10px] text-neutral-400">{{ PRODUCT_CATS[i] }}</p>
              <p class="mt-1 text-xs font-bold text-neutral-900">$99.00</p>
            </div>
          </div>
        </div>

        <!-- editorial -->
        <div v-else-if="str('card_style','shop') === 'editorial'" class="flex gap-5 overflow-hidden">
          <div v-for="(img, i) in PRODUCT_IMGS" :key="i" class="w-48 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
            <div class="h-28 w-full overflow-hidden bg-neutral-100">
              <div class="h-full w-full bg-cover bg-center" :style="{ backgroundImage: `url('${img}')` }" />
            </div>
            <div class="p-3">
              <p class="text-xs font-bold text-neutral-900">Product {{ i + 1 }}</p>
              <p class="mt-1 text-[10px] leading-relaxed text-neutral-500">A beautifully crafted product for modern living.</p>
              <button type="button" class="mt-2 rounded-lg bg-indigo-600 px-3 py-1.5 text-[10px] font-bold text-white">View</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid min-h-[400px] grid-cols-2">
        ...
        <div class="flex flex-col justify-center px-10 py-12">
          ...
          <p class="mt-4 text-sm leading-relaxed text-neutral-600">
            {{ str('description', 'Describe the product here. Highlight key features and benefits.') }}
          </p>
          <button type="button" class="mt-6 w-fit rounded-md bg-neutral-900 px-8 py-3 text-sm font-bold text-white">
            Add to cart
          </button>
        </div>
      </div>
    ...
    <template v-else-if="section.type === 'categories_grid'">
      <div class="px-8 py-12">
        <h3 class="mb-6 text-lg font-bold text-neutral-900">
          {{ str('heading', 'Shop by category') }}
        </h3>

        <!-- overlay style (default) -->
        <div v-if="str('card_style','overlay') === 'overlay'" class="grid grid-cols-4 gap-4">
          <div
            v-for="(img, i) in CAT_IMGS"
            :key="i"
        ...
        <!-- below style -->
        <div v-else-if="str('card_style','overlay') === 'below'" class="grid grid-cols-4 gap-4">
          <div v-for="(img, i) in CAT_IMGS" :key="i" class="space-y-2">
            <div
              class="aspect-square overflow-hidden rounded-xl bg-cover bg-center bg-neutral-200"
              :style="{ backgroundImage: `url('${img}')` }"
            />
            <p class="text-center text-sm font-semibold text-neutral-900">Category {{ i + 1 }}</p>
            <p v-if="section.settings.show_count" class="text-center text-xs text-neutral-500">24 products</p>
          </div>
        </div>
        ...
        <div v-else-if="str('card_style','overlay') === 'split'" class="grid grid-cols-2 gap-4">
          <div
            v-for="(img, i) in CAT_IMGS.slice(0, 4)"
            :key="i"
            class="flex h-36 overflow-hidden rounded-xl border border-neutral-200"
          >
            <div
              class="flex w-1/2 items-center justify-center p-4"
              :style="{ backgroundColor: CAT_COLORS[i] + '22' }"
            >
              <p class="text-sm font-bold text-neutral-900">Category {{ i + 1 }}</p>
            </div>
            <div
              class="w-1/2 bg-cover bg-center bg-neutral-200"
              :style="{ backgroundImage: `url('${img}')` }"
            />
          </div>
        </div>
    ...
    <template v-else-if="section.type === 'rich_text'">
      <div class="px-8 py-14 text-center">
        <h2 v-if="str('heading')" class="text-2xl font-bold text-neutral-900">
          {{ str('heading') }}
        </h2>
        <div class="mx-auto mt-4 max-w-2xl text-[15px] leading-relaxed text-neutral-600">
          {{ str('body', 'Start writing your content here. You can add rich text, format it and customize it in the settings panel.') }}
        </div>
      </div>
    </template>
      ...
      <div class="grid min-h-[280px] grid-cols-2">
        ...
        <div class="flex flex-col justify-center px-10 py-12">
          <h2 class="text-2xl font-bold text-neutral-900">
            {{ str('heading', 'Our story') }}
          </h2>
          <p class="mt-4 text-[15px] leading-relaxed text-neutral-600">
            {{ str('body', 'Tell your brand story, highlight what makes you special, and connect with your customers on a deeper level.') }}
          </p>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'image_gallery'">
      <div class="px-8 py-12">
        <h2 v-if="str('heading')" class="mb-6 text-xl font-bold text-neutral-900">
          {{ str('heading') }}
        </h2>
        <div class="grid grid-cols-3 gap-3">
          <div
            v-for="(img, i) in GALLERY_IMGS"
            :key="i"
            class="aspect-square overflow-hidden rounded-lg bg-cover bg-center bg-neutral-200"
            :style="{ backgroundImage: `url('${img}')` }"
          />
        </div>
      </div>
    ...
    <template v-else-if="section.type === 'banner_full'">
      <div class="flex min-h-[200px] items-center justify-center bg-neutral-900 px-8 py-12 text-center">
        <div>
          <h2 class="text-2xl font-bold text-white">
            {{ str('heading', 'Special Offer') }}
          </h2>
          <p class="mt-3 text-neutral-400">
            {{ str('subheading', 'Limited time deal — don\'t miss out') }}
          </p>
          <button type="button" class="mt-6 rounded-md bg-white px-8 py-2.5 text-sm font-bold text-neutral-900">
      ...
      <div class="grid min-h-[200px] grid-cols-2">
        <div class="flex items-center justify-center bg-neutral-900 p-10 text-center">
          <div>
            ...
            <button type="button" class="mt-4 rounded-md border border-white px-6 py-2 text-sm font-semibold text-white">
        ...
        <div class="flex items-center justify-center bg-neutral-100 p-10 text-center">
          <div>
            ...
            <button type="button" class="mt-4 rounded-md bg-neutral-900 px-6 py-2 text-sm font-semibold text-white">
    ...
    <template v-else-if="section.type === 'testimonials'">
      <div class="px-8 py-14">
        <h2 class="mb-8 text-center text-xl font-bold text-neutral-900">
          {{ str('heading', 'What our customers say') }}
        </h2>
        <div class="grid grid-cols-3 gap-6">
          ...
          >
            ...
            <p class="text-sm text-neutral-600">
              "Great product, highly recommended to everyone who values quality!"
            </p>
    ...
    <template v-else-if="section.type === 'trust_badges'">
      <div class="border-t border-neutral-200 px-8 py-10">
        <h3 class="mb-6 text-center text-xs font-bold tracking-widest text-neutral-400">
          {{ str('heading', 'Why shop with us') }}
        </h3>
    ...
    <template v-else-if="section.type === 'logo_bar'">
      <div class="border-t border-neutral-200 px-8 py-10">
        <p class="mb-8 text-center text-xs font-bold tracking-widest text-neutral-400">
          {{ str('heading', 'As seen in') }}
        </p>
    ...
    <template v-else-if="section.type === 'faq_accordion'">
      <div class="px-8 py-14">
        <h2 class="mb-8 text-center text-xl font-bold text-neutral-900">
          {{ str('heading', 'Frequently asked questions') }}
        </h2>
      ...
      <div class="bg-neutral-50 px-8 py-14 text-center">
        ...
        <div class="mt-6 flex justify-center gap-2">
          <input
            ...
            readonly
          />
          <button type="button" class="rounded-md bg-neutral-900 px-5 py-2 text-sm font-semibold text-white">
    ...
    <template v-else-if="section.type === 'contact_form'">
      <div class="px-8 py-14">
        ...
        <div class="mt-6 grid max-w-xl grid-cols-2 gap-4">
          <input type="text" placeholder="Name" class="rounded-md border border-neutral-300 px-4 py-2.5 text-sm" readonly />
          <input type="email" placeholder="Email" class="rounded-md border border-neutral-300 px-4 py-2.5 text-sm" readonly />
          <textarea placeholder="Message" rows="4" class="col-span-2 rounded-md border border-neutral-300 px-4 py-2.5 text-sm" readonly />
          <button type="button" class="w-fit rounded-md bg-neutral-900 px-6 py-2.5 text-sm font-semibold text-white">
    ...
    <template v-else-if="section.type === 'spacer'">
      <div class="relative" :style="{ height: `${num('height', 48)}px` }">
        <div
          v-if="selected || isHovered"
          class="absolute inset-0 flex items-center justify-center"
        >
          <span class="rounded-full bg-neutral-100 px-3 py-0.5 text-[10px] font-medium text-neutral-500">
            Spacer · {{ num('height', 48) }}px
          </span>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'custom_html'">
      <div class="flex flex-col items-center justify-center gap-2 px-8 py-10 text-center">
        ...
        <p class="text-xs text-neutral-400">Edit in settings panel to add HTML code</p>
      </div>
    </template>
...
const label = computed(() => SECTION_TYPE_LABELS[props.section.type] ?? props.section.type)
