<template>
  ...
  >
    ...
    <template v-if="section.type === 'hero_banner'">
      ...
      >
        <div class="relative z-10 max-w-xl">
          <h2
            class="text-3xl font-bold leading-tight tracking-tight text-white md:text-4xl lg:text-[2.75rem]"
          >
    ...
    <template v-else-if="section.type === 'hero_video'">
      <div
        class="relative flex min-h-[380px] items-center justify-center bg-neutral-900"
      >
        <Icon
          name="ph:play-circle-fill"
          class="absolute text-8xl text-white/20"
        />
    ...
    <template v-else-if="section.type === 'products_grid'">
      <div class="border-t border-neutral-200 px-8 py-12">
        ...
        <div
          v-if="
            str('card_style', 'shop') !== 'sale' &&
            str('card_style', 'shop') !== 'horizontal' &&
            str('card_style', 'shop') !== 'editorial'
          "
          class="mt-8 grid grid-cols-3 gap-5"
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
            :key="i"
          ...
          >
            ...
            <div class="mt-3">
              ...
              <p class="mt-0.5 text-sm font-bold text-neutral-900">
                Product {{ i + 1 }}
              </p>
              ...
              <div class="mt-3 flex items-center justify-between gap-2">
                <div>
                  ...
                  <span class="ms-1.5 text-xs text-neutral-400 line-through"
                    >$100</span
                  >
                </div>
                <button
        ...
        <div
          v-else-if="str('card_style', 'shop') === 'sale'"
          class="mt-8 grid grid-cols-3 gap-5"
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
        ...
        <div
          v-else-if="str('card_style', 'shop') === 'horizontal'"
        ...
        >
          ...
          >
            <div
              class="size-20 shrink-0 overflow-hidden rounded-xl bg-neutral-50"
            >
              <div
        ...
        <div
          v-else-if="str('card_style', 'shop') === 'editorial'"
          class="mt-8 grid grid-cols-3 gap-5"
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
      ...
      <div class="overflow-hidden px-8 py-12">
        ...
        >
          ...
          >
            ...
            >
              <div
                class="h-24 w-full bg-contain bg-center bg-no-repeat"
                :style="{ backgroundImage: `url('${img}')` }"
              />
            </div>
            <div class="mt-2">
              <p class="text-[10px] text-neutral-400">{{ PRODUCT_CATS[i] }}</p>
              <p class="mt-0.5 text-xs font-bold text-neutral-900">
                Product {{ i + 1 }}
              </p>
              <div class="mt-1 flex text-amber-400">
                ...
                <Icon
                  name="ph:star-bold"
                  class="text-[10px] text-neutral-300"
                />
              </div>
              <div class="mt-2 flex items-center justify-between gap-1">
                <div>
                  <span class="text-xs font-bold text-neutral-900">$80</span>
                  <span class="ms-1 text-[10px] text-neutral-400 line-through"
                    >$100</span
                  >
                </div>
                <button
        ...
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
            :key="i"
            class="relative w-48 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm"
          >
            <span
        ...
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
            :key="i"
            class="flex items-center gap-3 overflow-hidden rounded-2xl border border-neutral-200 bg-white p-3 shadow-sm"
          >
            <div
              class="size-14 shrink-0 overflow-hidden rounded-xl bg-neutral-50"
            >
              <div
                class="h-full w-full bg-contain bg-center bg-no-repeat"
                :style="{ backgroundImage: `url('${img}')` }"
        ...
        >
          <div
            v-for="(img, i) in PRODUCT_IMGS"
            :key="i"
            class="w-48 shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm"
          >
    ...
    <template v-else-if="section.type === 'categories_grid'">
      <div class="px-8 py-12">
        ...
        <div
          v-if="str('card_style', 'overlay') === 'overlay'"
        ...
        >
          ...
          >
            ...
            <div class="absolute inset-0 flex items-end p-3">
              <span
                class="rounded-md bg-white/90 px-2.5 py-1 text-xs font-semibold text-neutral-900"
                >Category {{ i + 1 }}</span
              >
            </div>
          </div>
        ...
        <!-- below style -->
        <div
          v-else-if="str('card_style', 'overlay') === 'below'"
          class="grid grid-cols-4 gap-4"
        >
          <div v-for="(img, i) in CAT_IMGS" :key="i" class="space-y-2">
            <div
              ...
              :style="{ backgroundImage: `url('${img}')` }"
            />
            <p class="text-center text-sm font-semibold text-neutral-900">
              Category {{ i + 1 }}
            </p>
            <p
              v-if="section.settings.show_count"
              class="text-center text-xs text-neutral-500"
            >
              24 products
            </p>
          </div>
        ...
        <!-- split style -->
        <div
          v-else-if="str('card_style', 'overlay') === 'split'"
    ...
    <template v-else-if="section.type === 'rich_text'">
      <div class="px-8 py-14 text-center">
        ...
        <div
          class="mx-auto mt-4 max-w-2xl text-[15px] leading-relaxed text-neutral-600"
        >
          {{
            str(
              "body",
    ...
    <template v-else-if="section.type === 'banner_full'">
      <div
        class="flex min-h-[200px] items-center justify-center bg-neutral-900 px-8 py-12 text-center"
      >
        <div>
          <h2 class="text-2xl font-bold text-white">
    ...
    <template v-else-if="section.type === 'banner_split'">
      <div class="grid min-h-[200px] grid-cols-2">
        <div
          class="flex items-center justify-center bg-neutral-900 p-10 text-center"
        >
          <div>
        ...
        <div
          class="flex items-center justify-center bg-neutral-100 p-10 text-center"
        >
          <div>
      ...
      <div class="bg-neutral-900 px-8 py-12 text-center">
        ...
        <div class="mt-6 flex justify-center gap-6">
          ...
          >
            ...
            <span
              class="text-[10px] font-medium tracking-widest text-neutral-500"
              >{{ unit.label }}</span
            >
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-6">
          ...
          >
            ...
            <p class="text-sm text-neutral-600">
              "Great product, highly recommended to everyone who values
              quality!"
            </p>
            <p class="mt-4 text-xs font-semibold text-neutral-800">
              — Happy Customer {{ i }}
            </p>
          </div>
        </div>
    ...
    <template v-else-if="section.type === 'trust_badges'">
      <div class="border-t border-neutral-200 px-8 py-10">
        <h3
          class="mb-6 text-center text-xs font-bold tracking-widest text-neutral-400"
        >
    ...
    <template v-else-if="section.type === 'logo_bar'">
      <div class="border-t border-neutral-200 px-8 py-10">
        <p
          class="mb-8 text-center text-xs font-bold tracking-widest text-neutral-400"
        >
        ...
        <div class="flex items-center justify-center gap-10 opacity-35">
          <div
            v-for="i in 5"
            :key="i"
            class="h-6 w-20 rounded bg-neutral-500"
          />
        </div>
      </div>
    </template>
    ...
    <template v-else-if="section.type === 'faq_accordion'">
      <div class="px-8 py-14">
        ...
        <div
          class="mx-auto max-w-2xl divide-y divide-neutral-200 border-t border-neutral-200"
        >
          <div
            v-for="i in 4"
            :key="i"
    ...
    <template v-else-if="section.type === 'video_embed'">
      <div class="px-8 py-12">
        <div
          class="mx-auto flex aspect-video max-w-3xl items-center justify-center rounded-xl bg-neutral-900"
        >
    ...
    <template v-else-if="section.type === 'spacer'">
      <div class="relative" :style="{ height: `${num('height', 48)}px` }">
        ...
        >
          <span
            class="rounded-full bg-neutral-100 px-3 py-0.5 text-[10px] font-medium text-neutral-500"
          >
    ...
    <template v-else-if="section.type === 'custom_html'">
      ...
      >
        ...
        <p class="text-xs text-neutral-400">
          Edit in settings panel to add HTML code
        </p>
      </div>
    </template>
    ...
    <template v-else>
      <div
        class="flex min-h-[72px] items-center gap-3 border-t border-neutral-200 px-8 py-5"
      >
