  ...
  >
    ...
    <div class="min-h-0 flex-1 overflow-auto rounded-lg">
      <preview-frame :device="store.device">
        <div class="flex h-full min-h-[520px] flex-col overflow-hidden rounded-t-xl bg-white">
          <!-- Browser chrome -->
          <div class="flex shrink-0 items-center gap-3 border-b border-neutral-200/90 bg-neutral-100/95 px-3 py-2.5">
            ...
            <div class="mx-auto flex max-w-md flex-1 items-center justify-center rounded-md bg-white/90 px-3 py-1.5 text-center text-[11px] text-neutral-400 shadow-sm ring-1 ring-neutral-200/80">
              <span class="truncate">mystore.storeos.io/{{ store.currentPage?.slug ?? 'home' }}</span>
            </div>
          ...
          <div class="relative min-h-0 flex-1 overflow-y-auto bg-white text-neutral-900">
            ...
            <order-summary-preview v-else-if="store.currentPage?.type === 'order_confirmed'" />

            <!-- ── Regular page ───────────────────────────────────── -->
            <template v-else>
              <!-- Persistent Navbar -->
              <navbar-preview :selected="navbarSelected" @select="selectNavbar" />
              ...
              >
                <Icon name="ph:squares-four-bold" class="text-4xl text-neutral-300" />
                <p class="mt-3 text-sm text-neutral-400">No sections yet</p>
                <p class="text-xs text-neutral-300">Add a section from the left panel</p>
              </div>

              <!-- Persistent Footer -->
              <footer-preview :selected="footerSelected" @select="selectFooter" />
            </template>
          </div>
        </div>
      </preview-frame>
...
function onToolbar(action: string, sectionId: string) {
  switch (action) {
    case 'up': store.moveSection(sectionId, 'up'); break
    case 'down': store.moveSection(sectionId, 'down'); break
    case 'dup': store.duplicateSection(sectionId); break
    case 'hide': store.toggleSectionHidden(sectionId); break
    case 'del': store.removeSection(sectionId); break
  }
}
</script>
