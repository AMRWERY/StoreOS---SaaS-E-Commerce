    ...
    <div class="max-h-[min(60vh,520px)] space-y-6 overflow-y-auto px-5 py-5 xl:max-h-[calc(100vh-14rem)]">
      ...
      <setting-group title="Navbar" default-open>
        ...
        <div class="mt-5 space-y-2">
          ...
          <div v-if="store.navLinks.length === 0" class="rounded-lg border border-dashed border-border-subtle py-6 text-center">
            <p class="text-[11px] text-tx-muted">No nav links yet.</p>
            <button type="button" class="mt-2 text-[11px] font-semibold text-brand hover:underline" @click="store.addNavLink()">
              Add your first link
            </button>
          </div>
        </div>
